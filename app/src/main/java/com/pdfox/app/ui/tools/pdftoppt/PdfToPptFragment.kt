...
@AndroidEntryPoint
class PdfToPptFragment : BaseToolFragment() {

    private val viewModel: PdfToPptViewModel by viewModels()
            ...
            viewModel.uiState.collectLatest { state ->
                when (state) {
                    ...
                    is ToolUiState.Error -> {
                        Timber.e(state.exception, "PDF to PPT failed: ${state.message}")
                    }
                }
            }
        }
    ...
    override fun getToolId(): String = "pdf_to_ppt"
